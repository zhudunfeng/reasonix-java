package com.reasonix.agent.compact;
...
public class CompactService {
    ...
    public void compact(Session session) {
        ...
        java.util.List<com.reasonix.agent.model.ChatMessage> history = session.getHistory();
        if (history.size() <= 2) {
            return;
        }
        int keep = Math.max(1, history.size() / 2);
        java.util.List<com.reasonix.agent.model.ChatMessage> compacted = new java.util.ArrayList<>(history.subList(history.size() - keep, history.size()));
        history.clear();
